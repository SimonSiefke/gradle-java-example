...
// this.clusterAssignments = new int[N]; // maps data point indices to cluster
// indices
// this.clusterCenters =
...
// Arrays.stream(clusterCenters).map(Cluster::new).toArray(Cluster[]::new);
// for (int n = 0; n < N; n++) {
...
// double minDistance;
// if (r[n]) {
// minDistance = distance.compute(dataPoints[n],
...
// // step 3b
// if (minDistance > lowerBounds[n][k] || minDistance > 0.5 *
// interClusterDistances[clusterAssignments[n]][k]) {
...
// // step 4
// hasChanged = Util.updateClusterCenters(clusterCenters, newClusterCenters,
// clusterAssignments, dataPoints);

// // step 5
// for (var lowerBound : lowerBounds) {
// for (int k = 0; k < K; k++) {
// lowerBound[k] = Math.max(lowerBound[k] - distance.compute(clusterCenters[k],
// newClusterCenters[k]), 0);
// }
// }
