 ...
 */
public class KPlusPlusClusterCenterInitializationStrategy implements ClusterCenterInitializationStrategy {
  @Override
  public double[][] initialize(@Nonnull double[][] dataPoints, int K, @Nonnull DistanceStrategy distance) {
    ...
    var weightedDistribution = new double[K]; // cumulative sum of squared distances
    ...
    for (int k = 0; k < K; k++) {
      ...
      // after first centroid, use a weighted distribution
        ...
        // check if the most recently added centroid is closer to any of the points than
        // previously added ones
        for (int l = 0; l < K; l++) {
          // gives chosen points 0 probability of being chosen again -> sampling without
          // replacement
          double tempDistance = distance.compute(dataPoints[l], centroids[k - 1]); // need L2 norm here, not L1
