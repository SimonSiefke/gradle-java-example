...
public class HelloBenchmark {
   ...
   *
   * The contract for the benchmark methods is very simple: annotate it
   * with @Benchmark, and you are set to go. JMH will run the test by continuously
   * calling this method, and measuring the performance metrics for its execution.
   *
   ...
   * well. If you throw the exception from the method body, the JMH run ends
   * abruptly for this benchmark, and JMH will run the next benchmark down the
   * list.
   *
   * Although this benchmark measures "nothing", it is the good showcase for the
   * overheads the infrastructure bear on the code you measure in the method.
   * There are no magical infrastructures which incur no overhead, and it's
