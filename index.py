...
class Kmeans:
    ...
    def selectSeeds(self, k):
        seeds = [self.pointList[0], self.pointList[1]]
    ...
    def getCentroidInit(self, point):
        ...
        if point.centroid is not None:
            dist = self.getDistance(
                point, self.centroidList[point.centroid].point)
            minDist = dist
    ...
    def reCalculateCentroid(self):
        ...
        for centroid in self.centroidList:
            ...
            for x in range(0, self.dim):
                try:
                    mean.coordinates[x] = mean.coordinates[x] / \
                        len(centroid.pointList)
                except:
    ...
    def calcInterCluster(self):
        for i in range(0, self.k):
            for j in range(i+1, self.k):
                temp = self.getDistance(
                    self.centroidList[i].point, self.centroidList[j].point)
        ...
        for i in range(len(self.pointList)-1, -1, -1):
            ...
            if self.pointList[i].centroid is None:
                ...
                self.centroidList[centroidPos].pointList.append(
                    copy.deepcopy(self.pointList[i]))
    ...
    def assignPoints(self):
        ...
        for x in range(self.k):
            self.distanceMap[x] = self.getDistance(
                self.oldCentroid[x], self.centroidList[x].point)
        for i in range(len(self.centroidList)-1, -1, -1):
            for j in range(len(self.centroidList[i].pointList)-1, -1, -1):
                try:
                    ...
                    self.upperBound[self.centroidList[i].pointList[j]
                                    .id] += self.distanceMap[self.centroidList[i].pointList[j].centroid]
                    ...
                    if self.upperBound[self.centroidList[i].pointList[j].id] > self.minimumClusterDistance[self.centroidList[i].pointList[j].centroid]:
                        temp = self.getCentroid(
                            self.centroidList[i].pointList[j])
                        centroidPos = temp[0]
                        centroidDist = temp[1]
